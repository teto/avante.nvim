...
cmd("SwitchProvider",
  function(opts)
    -- require("avante.api").switch_provider(vim.trim(opts.args or "")) end
    -- local prefix = line:match("AvanteSwitchProvider%s*(.*)$") or ""
      -- vim.tbl_keys(Config.providers)
    vim.print(opts.args)

    local get_providers = function(prefix)
    ...
    vim.ui.select(get_providers(prefix), { prompt = "Provider> " }, function (choice, idx)
        if idx ~= nil then
