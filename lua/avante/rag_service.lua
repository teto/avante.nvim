...
function M.launch_rag_service(cb)
  ...
  elseif M.get_rag_service_runner() == "nix" then
    ...
    Utils.debug(string.format("Checking if RAG service is active at %s", service_path))
    ...
    --   "ALLOW_RESET=TRUE PORT=%d DATA_DIR=%s RAG_EMBED_PROVIDER=%s RAG_EMBED_ENDPOINT=%s RAG_EMBED_API_KEY=%s RAG_EMBED_MODEL=%s RAG_EMBED_EXTRA=%s RAG_LLM_PROVIDER=%s RAG_LLM_ENDPOINT=%s RAG_LLM_API_KEY=%s RAG_LLM_MODEL=%s RAG_LLM_EXTRA=%s sh run.sh %s",
    --   ,
    --   service_path,
    --   Config.rag_service.embed.provider,
    --   Config.rag_service.embed.endpoint,
